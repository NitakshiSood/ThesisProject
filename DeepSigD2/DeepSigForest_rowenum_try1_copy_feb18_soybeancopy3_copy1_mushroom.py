...
#this one works with 100 sig inn each esgi.
import random
import numpy as np
...
sum_dictvalues=0;
count1={};
def cascade_op_valstep(new_val_train,new_val,n,df_y_train,df_y_val,test_name,total_copies_ineachEsig):
    ...
    finalchunksublist,finalchunksublist_new_val=bagged_chunks(new_val_train,new_val, n,total_copies_ineachEsig)
...
def cascadestep2_valstep(new_val_train,new_val,n,df_X_val,df_X_train,df_y_train,df_y_val,test_name,dataset_name,df2_slidingwindow,df3_slidingwindow,layer,num_layers,flagstage,total_copies_ineachEsig): 
    print("GOES HERE?")
    ...
    flag=0
    lastlayer_pred_list=[]
    df3_concat=pd.DataFrame()
    df2_concat=pd.DataFrame()
    for num in range(num_sigforests):
        cascade_op_valstep(new_val_train_original,new_val_original,n,df_y_train_original,df_y_val_original,test_name,total_copies_ineachEsig)
        #cascade_op(new_val_train,new_val,n,df_y_train,df_y_val,test_name)
        #calling a subsig
        for subsig in range(total_copies_ineachEsig):#total_copies_ineachEsig
            ...
            print("?????@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2")
        print("appending")
        print("now adding 4 sig results")
        df3_concat=pd.concat([df3_concat,df3], axis=0, sort=False)
        df2_concat=pd.concat([df2_concat,df2], axis=0, sort=False)
    #print("df3_concat: ",df3_concat)
    #print("df2_concat: ",df2_concat)
    #print(stopp)        
    df2=df2_concat
    df3=df3_concat
    #if layer
    ...
    new_val=pd.concat([df2_slidingwindow, df2,df_y_val], axis=1, sort=False) # ihave to do the avg here
    ...
    print("df3_slidingwindow.shape: ",df3_slidingwindow.shape)
    ...
    new_val_train= pd.concat([df3_slidingwindow, df3,df_y_train], axis=1, sort=False)    
    
    print("new_val_train.shape: ",new_val_train.shape)
    #print(stoppppp)
...
def subsample(dataset, ratio):
    ...
    return dataset
def bagged_chunks(my_list,new_val, n,total_copies_ineachEsig):
    #total_copies_ineachEsig=5 #each esig has 100 forests
    ...
    finalchunksublist=[]
    for i in range(total_copies_ineachEsig):
        final = [list_in[i * n:(i + 1) * n] for i in range((len(list_in) + n - 1) // n )]
        print("final: ",final)
        li=random.sample(final, 1)#random list. ensuring randomness
        print("li: ",li)
...
if __name__ == "__main__":
    ...
    fileName='mushroomchanged'#'letRecog'#'adult_arff_nomissingval'#'ionosphere'#'yeast_csv_changed'#'adult_arff_nomissingval'#'adult_myversion2_nomissingdata'#'adult_myversion2_nomissingdata'#'ionosphere'#'penDigits'#'mushroomchanged'
    ...
    for num_run in range(1,num_runs+1):
        ...
        #x_train_subset=[]
        #Xtest_sub=[]
        windowsize_list=[8,9,10]#[7,8,9]#[14,15,16]#[5,6,7]#[8,9,10]#[8,9,10]#[5,6,7]#[8,9,10] # this code works only for three types of window sizes
        ...
        n=10 # here n is the number of random samples you generate. most probably check again
        ...
        print("df2_slidingwindow.shape: ",df2_slidingwindow.shape) # this is actually the output equal to f total
        print("df3_slidingwindow.shape: ",df3_slidingwindow.shape)
        print("df4_slidingwindow.shape: ",df4_slidingwindow.shape)
        #print(stopp)
        ############################
        new_val_train.columns = np.arange(len(new_val_train.columns))
        new_val.columns = np.arange(len(new_val.columns))
        print("new_val_train",new_val_train)
        print("new_val",new_val)
        print("df_X_val",df_X_val)
        print("df_X_train",df_X_train)
        print("df_y_train",df_y_train)
        print("df_y_val",df_y_val)
        df2_slidingwindow.columns = np.arange(len(df2_slidingwindow.columns))
        df3_slidingwindow.columns = np.arange(len(df3_slidingwindow.columns))
        df4_slidingwindow.columns = np.arange(len(df4_slidingwindow.columns))        
        print("df2_slidingwindow",df2_slidingwindow)
        print("len: df2_slidingwindow: ",df2_slidingwindow.axes[0],df2_slidingwindow.axes[1],df2_slidingwindow.info(),len(df2_slidingwindow),len(df2_slidingwindow.columns))
        print("df3_slidingwindow",df3_slidingwindow)
        print("df4_slidingwindow",df4_slidingwindow)
        ...
        #tname  =  'garbage/' +  dataset_name + 'funout'+'.csv'
        #df4_slidingwindow.to_csv(tname)        
        total_copies_ineachEsig=50    
        #print(weighstop)
        ...
        #for train set
            ...
            new_val_train,df_y_train=dataset_transformation_mgstage1.main(new_val_train,original_feature_length,df_y_train)# you dont need original_feature_length, orifinally u appended the original feature vector which is wrong and thats why u had it. but in the code i have made this original_feature_length =0
            #print("after new_val_train: ",new_val_train)
            #print(checkthisone)
            #print("before new_val_train: ",new_val)
            ...
            #print(stophere3)  
            new_val_train,new_val,lastlayer_pred_list=cascadestep2_valstep(new_val_train,new_val,n,df_X_val,df_X_train,df_y_train,df_y_val,test_name,dataset_name,df2_slidingwindow,df3_slidingwindow,layer,num_layers,flagstage,total_copies_ineachEsig)
            #print(stopppppppppppppppppppp)
        ...
        #for train set
            ...
            #print(stophere3)            
            new_test,new_val,lastlayer_pred_list=cascadestep2_valstep(new_test,new_val,n,df_X_val,df_X_test,df_y_test,df_y_val,test_name,dataset_name,df2_slidingwindow,df4_slidingwindow,layer,num_layers,flagstage,total_copies_ineachEsig)
            #print("new_val_train232: ")
            #print(new_test)
            #print("new_val232: ")
            #print(new_val)
