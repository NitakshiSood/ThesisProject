...
sum_dictvalues=0;
count1={};
def cascade_op_valstep(new_val_train,new_val,n,df_y_train,df_y_val,test_name,df_X_val,df_X_train,df2_slidingwindow,df3_slidingwindow,num_layers,flagstage,layer):
    ...
    count=1
    df1_train,df2,df3_train,df4=dataset_transformation.main(new_val_train,df_y_val,new_val,df_y_train)
    print("df1_train: ",df1_train) # this is smaller in size.
    print("df2: ",df4)
    Xtotal_subsample = pd.concat([df1_train,df4], axis=1)
    Xtest_total=Xtotal_subsample.values.tolist()
    test_name  = 'subset_datasetsnew/' + dataset_name + '_train'+ str(count) +'.txt' #litrelly trainset
    ...
    #####testttttt@@@@@@@@@@
    print("df3_train: ",df3_train) #this is larger in size
    print("df4: ",df2)
    Xtotal_subsample = pd.concat([df3_train,df2], axis=1)
    Xtest_total=Xtotal_subsample.values.tolist()
    trainfile  = 'subset_datasetsnew/' + dataset_name + '_val'+ str(count) +'.txt'
    ...
    with open(trainfile, "w",newline='\n') as f:
        ...
        writer.writerows(Xtest_total)
    #print(stopppppp)
    #newcascade(new_val_train,new_val,n,df_y_train,df_y_val,test_name)
    new_val_train,new_val,lastlayer_pred_list=newcascade(trainfile,test_name,df1_train,df2,df3_train,df4,df_X_val,df_X_train,df2_slidingwindow,df3_slidingwindow,df_y_train,df_y_val,num_layers,flagstage,layer)
...
def newcascade(train_name,test_name,df1_train,df2,df3_train,df4,df_X_val,df_X_train,df2_slidingwindow,df3_slidingwindow,df_y_train,df_y_val,num_layers,flagstage,layer):
    print("i am in  new cascade ")
    new_val_train,new_val,lastlayer_pred_list=CSFSS.main(train_name,test_name,df1_train,df2,df3_train,df4,df_X_val,df_X_train,df2_slidingwindow,df3_slidingwindow,df_y_train,df_y_val,num_layers,flagstage,layer)
    #print(stop)
    return new_val_train,new_val,lastlayer_pred_list
...
def cascadestep2_valstep(new_val_train,new_val,n,df_X_val,df_X_train,df_y_train,df_y_val,test_name,dataset_name,df2_slidingwindow,df3_slidingwindow,layer,num_layers,flagstage): 
    ...
    num_sigforests=2 #3 #number of sigs you want in cascade step #SET THIS
    ...
    for num in range(num_sigforests):
        print("new_val_train_original",new_val_train_original) #this is val set as it is smaller
        print("new_val_original",new_val_original)
        print("df_y_train_original",df_y_train_original) #this is bigger so train set
        print("df_y_val_original",df_y_val_original)
        
        new_val_train,new_val,lastlayer_pred_list=cascade_op_valstep(new_val_train_original,new_val_original,n,df_y_train_original,df_y_val_original,test_name,df_X_val,df_X_train,df2_slidingwindow,df3_slidingwindow,num_layers,flagstage,layer)
...
def bagged_chunks(my_list,new_val, n):
    ...
    print("NOW list_in: ",list_in)
    print()
    final = [list_in[i * n:(i + 1) * n] for i in range((len(list_in) + n - 1) // n )]
    print("NOW FINAL: ",final)
    print()
...
def cascadestep2(new_val_train,new_val,n,df_X_val,df_X_train,df_y_train,df_y_val,test_name,dataset_name,df2_slidingwindow,df3_slidingwindow): 
    ...
    for num in range(num_sigforests):
        ...
        #you get the accumulated results here.
...
if __name__ == "__main__":
    ...
    for num_run in range(1,num_runs+1):
        ...
        #x_train_subset=[]
        #Xtest_sub=[]
        windowsize_list=[10,11,12]#[8,9,10]#[5,6,7]#[8,9,10]#[8,9,10]#[5,6,7]#[8,9,10] # this code works only for three types of window sizes
        original_feature_length=X_train.shape[1]
        print("CHECK THE SIZE HERE: ",X_train.shape)
        flag=0
        ...
        for window_size in windowsize_list:
            ...
            print("CALLING ASSOCIATIVE CLASSIFIER for window size: ",window_size)
            #print(weigh)
            #df1=pd.Series([])
            print("len(Xval_sub): ",len(Xval_sub))
            print("len(Xtrain_sub): ",len(Xtrain_sub))
            ...
            for sig_num in range(len(Xval_sub)):
                print("**************************total runs: curr RUN NUMBER: window size: ",len(Xval_sub),sig_num,window_size)
                #print(weigh)
                #val/test
        ...
        print("new_val: ",new_val)
        print("new_val.shape: ",new_val.shape)
        print("SHAPES1: ", new_val.shape,new_val_train.shape)
        print("*******")
        #print(stopp)
