...
def run_test(fold_number,input_name,test_name,dataset_name):
    sep = ' '
    ...
    t1, t2 = clf.fit(file_name=input_name, sep=sep, fold_number=fold_number,dataset_name=dataset_name)
    g_rules_count += t1
    np_rules_count += t2
    ...
    for hrs in [1,2,3]:
        ...
        if(max_accs<accs[hrs-1]/df.shape[0]):
        #if(max_accs<accs[hrs-1]):
            max_accs = accs[hrs-1]/df.shape[0]
            predictions_best=predictions
        print('accuracy S'+str(hrs)+' is:', accs[hrs-1]/df.shape[0])   
    #print("predictions_best: ",predictions_best)
    #print(weigh)
...
def main(dataset_name,counter):
    ...
    input_name  = 'datasets_bagging/' + dataset_name + '_train'+ str(counter)+'.txt'
    test_name   = 'datasets_bagging/' + dataset_name + '_test' + str(counter) + '.txt'
    print("input_name in newsig boosting: ",input_name)
    print("test_name in new sig boosting: ",test_name)
    print("KITNI BAR?")
    print(counter)
    accs,predictions = run_test(str(counter),input_name,test_name,dataset_name)
    acc_acc.append(accs)
    print('\n\n')
    sys.stdout.flush()
    ...
    hrs_acc={}    
    for hrs in [1,2,3]:
        acc=sum([x[hrs-1] for x in acc_acc])/test_count
    ...
    return predictions,max_acc,hrs_acc