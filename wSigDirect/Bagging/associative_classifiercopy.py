...
class AssociativeClassifier:
    ...
    def back_to_original_id(self, rules):
        new_rules = []
        for rule in rules:
            temp = sorted([ (self._id_item_dict[x]) for x in rule.get_items()],key=lambda x:(x)) #my change x:int(x) ->x:(x), int(self._id_item_dict[x] ->(self._id_item_dict[x]
    ...
    def _prune_rules(self):
        ...
        for transaction in self._database.get_transactions():
            ...
            transaction_items_set = set([(x) for x in items]) #my change int(x)
            max_confidence = -1.0
            max_rule = None
