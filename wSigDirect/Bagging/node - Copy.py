...
class Node:
    ...
    def has_label(self, label_):
        return self._labels.count((label_))>=1 #my change int(label_)

    def add_rule_node(self, label_):
        self._labels.append(int(float(label_))) #my change int(label_)
        self._rule_nodes.append(RuleNode())

    def get_rule_node(self, label_):
        place = self._labels.index(float(label_)) #my change int(label_)
