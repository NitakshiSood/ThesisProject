...
def run_test(fold_number,input_name,test_name):
    ...
    return list(map(lambda x:x/df.shape[0], accs)),predictions
...
def main(dataset_name,var_i,counter):
    acc_acc = []
    ...
    input_name='datasets_new_ensemble_boosting/' + dataset_name + '_train_specificfeatures'+ str(var_i)+str(counter) +'.txt' 
    test_name= 'datasets_new_ensemble_boosting/' + dataset_name + '_test_specificfeatures'+ str(var_i)+str(counter) +'.txt'
