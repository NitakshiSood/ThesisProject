...
def run_test(fold_number,input_name,test_name):
    ...
    return list(map(lambda x:x/df.shape[0], accs))
...
def main(dataset_name):
    ...
    for fold_number in range(1,test_count+1):
        input_name  = 'uci/' + dataset_name + '_tr'+ str(fold_number) +'.txt'
        test_name   = 'uci/' + dataset_name + '_ts' + str(fold_number) + '.txt'
        print(str(fold_number)+str(input_name)+str(test_name))
        
        accs = run_test(str(fold_number),input_name,test_name)
        acc_acc.append(accs)
        print('\n\n')
        sys.stdout.flush()
