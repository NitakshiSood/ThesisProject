...
class SigDirect:
    ...
    def fit(self, X, y, X_val,y_val, prune=None):
        ...
        while new_depth<len(x_set):
            ...
            logging.info('RULES: {:7}, MEM: {:6}MB, TIME: {:8.2f}'.format(len(generated_rules), 
                                                                        int(psutil.Process().memory_info().rss/(10**6)),
                                                                        time.time()-tt2, 
                                                                        ))
            logging.info('#############################################')
        ...
        return len(generated_rules), len(self._final_rules)
    ...
    def prune_ripper(self,generated_rules):
        ...
        while _rule_dict and _transactions: # try using this for sorting with confidence values
            ...
            print("conf: ",conf)
            #print(stopp)
            if not conf or conf < 0.50: # last was 30 which gave good results #0.50
                break            
    ...
    def _prune_rules(self, generated_rules):
        ...
        #    print("rule: ",str(rule))
        generated_rules=self.prune_ripper(generated_rules)#pruneripper(self,generated_rules)
