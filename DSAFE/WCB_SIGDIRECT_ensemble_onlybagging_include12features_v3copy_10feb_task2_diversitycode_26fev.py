...
def split(data, n):   
    ...
    for i in range(0, n*size, size):
        yield data[i:i+size]
def isDiverse(X_train_sub,diverseSets,T_over):
    var=True
    ...
    if len(diverseSets)==0 :
        ...
        for var in diverseSets.keys():  
            ...
            X_train_subcopy=X_train_sub.sort_values(by=0,axis=1, ascending=True)
            ...
            #print(stopphere)
            print("COUNT OF INTERSECTION2: ",len(X_train_sub.merge(diverseSets[var])),X_train_sub.shape[0],X_train_sub.shape[1])
...
def GenerateDiverseSets(X_train,Xtotal_subsample,X_test,Xval_subsample,RS, RF, T_over, sr):
    ...
    while change<sr:# or count!=4:
        ...
        #print("sample: ",sample, type(sample),class_label_col)
        ...
        #test set ends
        #train_name  =   'datasets_bag_rc/' + dataset_name + '_train'+ str(count) +'.txt'
        #X_train_sub.to_csv(train_name,sep=' ')
        '''
        if isDiverse(X_train_sub,diverseSets,T_over)==True:
            diverseSets[count]=X_train_sub
            train_name  =   'datasets_bag_rc/' + dataset_name + '_train'+ str(count) +'.txt'
            test_name  =   'datasets_bag_rc/' + dataset_name + '_test'+ str(count) +'.txt'
        ...
        if isfeatureframeDiverse(X_train_sub,diverseSets,T_over):
            print("yess diverse: ")
            diverseSets[count]=X_train_sub
            train_name  =   'datasets_bag_rc/' + dataset_name + '_train'+ str(count) +'.txt'
            test_name  =   'datasets_bag_rc/' + dataset_name + '_test'+ str(count) +'.txt'
...
if __name__ == "__main__":
    ...
    fileName='letRecog'#soybean_nomissingfile'#'ionosphere'#cylband_nomissingdata5'#'penDigits'#'cylband_nomissingdata5'#'ionosphere'#soybean_nomissingfile'#'ionosphere'#soybean_nomissingfile'#adultnew'#ionosphere'#adult'#'edited_WATT_Data7_March14_2018_preprocessed_NumNomConvert_3divnorm' #WCB_V5_norm _copy
